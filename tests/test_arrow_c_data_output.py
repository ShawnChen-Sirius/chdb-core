...
Arrow C Data Interface stream with pyarrow's `_import_from_c`, and compare
against `chdb.query(sql, "ArrowStream")` as the equivalence oracle.

Requires libchdb.so built from a NOT USE_PYTHON configuration. If libchdb.so
is missing or built without the new symbols, the whole test class is skipped.
"""
...
HAS_CHDB_PY = True  # Kept for the existence-skip predicate below.
...
def _load_libchdb():
    ...
    for path in _candidate_libchdb_paths():
        ...
        try:
            ...
            last_err = e
            continue
        # Require the new Arrow C Data Interface output symbols.
        if not hasattr(lib, "chdb_query_arrow"):
            continue
        return lib, path
    return None, last_err


LIBCHDB, LIBCHDB_PATH = _load_libchdb()
...
if LIBCHDB is not None:
    _bind(LIBCHDB)


@unittest.skipUnless(LIBCHDB is not None and HAS_CHDB_PY,
                     f"libchdb.so with chdb_query_arrow not available "
                     f"(loaded: {LIBCHDB_PATH}); chdb python: {HAS_CHDB_PY}")
class TestArrowCDataOutput(unittest.TestCase):
    ...
    def tearDownClass(cls):
        # We intentionally do NOT call chdb_close_conn here. The libchdb
        # connection-shutdown path joins worker threads that, with the
        # ctypes shim, never finish their handshake — the test process
        # would hang on teardown even though every test assertion passed.
        # Python exits cleanly via os._exit at interpreter shutdown.
        pass
    ...
    def test_integers_value_correctness(self):
        # We do NOT use chdb.query as an oracle here: it loads its own
        # pybind11-linked libchdb under /chdb/_chdb.abi3.so and instantiates
        # a parallel clickhouse_local global state in the same process.
        # Two parallel instances deadlock on engine globals. Instead we
        # value-check the result of our own C ABI directly.
